...
Também serão apresentados modelos mais recentes, como as funções radiais de base (seção
...
da parsemonia}} que basicamente detemina que o modelo deve ser o mais simples possível.
\cite{aguirre_maps}

Uma das primeiras justificativas para o desenvolvimento de metodos para a escolha de modelos é a
grande dificuldade de trablhar com modelos muito grandes e complexos. Existe o problema destes modelos
...
modelo sobreestimado. \cite{aguirre_jacome}
...
Estes modelos são normalmente utilizados em situações  onde a dinâmica do sistema pode ser bem
...
Hammerstein. Tem-se que a função estática não linear $f(\cdot)$ pode ser parametrizada
...
até para sistemas pouco não lineares, o número de parâmetros a estimar é grande. Isso
é devido ao fato da série tentar explicar a saída do sistema apenas baseado nos valores
da entrada deste.
...
A saída do sistema para uma rede multicamadas pode ser descrito como em
...
apenas uma camada adicionada de uma realimentação com um atraso de tempo como apresetado na Figura
...
Em função de terem uma estrutura mais floxivel, os modelos racionais podem vir a ser
...
Portanto a parte não deterministica de \eqref{eq:nl_model_narmax_pol_espec} pode ser espandida como
...
Sendo que $\theta_{nj}$ e $\theta_{dj}$ são os parametros dos regressores, possuindo informações até
o instante $t-1$. Desta forma o valor de parametros a ser estimados é $N_n + N_d$
...
média zero. Este é de certa forma o preço que se paga ao linearizar os parametros para que seja
possível utilizar o método dos minimos quadrados. \cite{aguirre}
