...
Identificação de sistemas é dividido em três partes básicas: o sistema real, a classe de modelos escolhida e o critério
de performance $V(\theta)$. No capítulo sobre identificação de sistemas lineares (Capítulo
\ref{chapter:system_identification}),  foi apresentado um dos critérios mais utilizados, que mede o custo de uma
função baseado no erro entre os dados observados do sistema real e a saída do modelo. Para projeto de controladores
existem desdobramentos deste critério em função dos objetivos básicos de controle:
...
\end{equation}
onde $y(t)$ é a saída medida do sistema e $y_d(t)$ é a saída desejada, $T_d(z)$ é o comportamento desejado para o
...
\begin{equation}
T_e(z)= 1-\frac{1}{H_0(z)} 
\label{eq:dbcd_perf_te}
\end{equation}

Da onde obtém-se o controlador desejado para a rejeição ao ruído $C_e(z)$:

\begin{equation}
C_e(z)=\frac{T_e(z)}{G_0(z)(1-T_e(z))}=\frac{H_0(z)-1}{G_0(z)} 
\label{eq:dbcd_perf_ce}
\end{equation}

O controlador apresentado em \eqref{eq:dbcd_perf_ce} também é conhecido como controlador da minima variância.
