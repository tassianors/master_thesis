...
\begin{equation}
\epsilon_L (t)=L(z)\epsilon (t)=\bar{r}_L (t)-y_L (t) 
\nonumber
\end{equation}
...
Aplicando o teorema de Perseval a função custo de seguimento de referência:
...
\begin{equation}
J_{VR}(\theta)=\bar{E}\left [ L(z)(u(t)-C(z,\theta)\bar{\epsilon}(t)) \right ]^2
\nonumber
\end{equation}
...
integrandos são diferentes, não existe motivos para esperar que os dois custos sejam os mesmos.
...
Observa-se a aproximação do valor de mínimo das funções custo pelo uso do filtro ($J_{VR}^L(\theta)$).}
...
$J_{VR}(\theta)$ (equação \eqref{eq:dbcd_vrft_jvr_cost}) observa-se que há a adição de um termo dentro do integrador, 
...
\nonumber
\end{equation}
e que este termo não decai a zero mesmo que $C_d(z) \in C(z,\theta)$. Desta forma é esperado que haja erro de
...
lineares modelados pelas classes de modelos ARX e BJ quando o controlador ideal $C_d(z)$ faz parte da classe de
controladores $C(z,\theta)$ e também um caso onde o controlador ideal não consegue ser presentado pela classe de
...
O vetor de parâmetros $\theta_0$ deste sistema é definido por $[ 0.4 \; -0.68 \; 0.288 ]$.
...
primeiro muito mais próximo da função $T_d(z)$ desejada. Os custos destes dois sistemas são apresentados na Tabela
\ref{table:vrft_method_notinclass}.
