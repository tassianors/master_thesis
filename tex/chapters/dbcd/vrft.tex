...
controlador, como os sinais $u(t)$ e $\varepsilon(t)$ são conhecidos, a tarefa reduz-se a um problema de
...
\item Selecione o vetor de parâmetros do controlador $\hat{\theta}$ que minimize o critério
...
Com o intúito de exemplificar, na Figura \ref{fig:vrft_cost_functions} é apresentado uma curva da função custo $J_y(\theta)$ de um
sistema hipotético, e a função custo proposta pelo método VRFT $J_{VR}(\theta)$, a qual é quadrática em $\theta$.
Ao tentar encontrar o ponto de mínimo da curva $J_y(\theta)$ pode-se recair em algum mínimo local, o que não é verdade
...
$\theta_d$ é o ponto de mínimo de ambas as funções custo, logo, minimizando a função custo $J_{VR}(\theta)$ é o
...
\theta)$ não consegue representar a totalidade do controlador ótimo ($C_d(z)$) que quando aplicado em conjunto com a
planta proporciona a exata resposta desejada pela escolha de $T_d(z)$.
...
O critério $J_{y}(\theta)$ pode ser reescrito utilizando-se o controlador ótimo $C_d(z)$ como em:

\begin{equation}
J_{y}(\theta)= \frac{1}{2\pi} \int_{-\pi}^{\pi} \frac{\left | G \right |^2 \left | W \right |^2 }
{\left | 1+GC(\theta) \right |^2} 
\frac{\left |C(\theta)-C_d  \right |^2}{\left |1+GC_d \right |^2}d\omega 
...
Utilizando as definições de $u_L(t)$, $\varepsilon_L(t)$ e dada a definição de $C_d(z)$ juntamente com o
teorema de Perseval \cite{ljung}, o critério assintótico \eqref{eq:vrft_method_filter_criter_assim}
tem sua representação como em:

\begin{equation}
J_{VR}(\theta)= \frac{1}{2\pi} \int_{-\pi}^{\pi} \left | G \right |^2 \left | C(\theta)-C_d \right
...
Obtém-se com a utilização do filtro, uma estimativa onde o erro de polarização é minimizado. Observa-se na figura
\ref{fig:vrft_cost_functions_L} um exemplo hipotético, onde o valor de $\theta$ que minimiza $J_{VR}(\theta)$ se
