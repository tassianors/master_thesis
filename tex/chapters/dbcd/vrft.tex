...
estes dois métodos pertencendo uma a mesma classe, algumas diferenças significativas existem e podem ser ressaltadas:
\cite{campi_savaresi2000}
...
Com o intuito de exemplificar, na Figura (\ref{fig:vrft_cost_functions}) é apresentado uma curva da função custo
...
Resultam na seguinte expressão no domínio da frequência para a função custo:
...
integrandos são diferentes, não existe motivos para esperar que os dois custos desejam o mesmo.
...
Com o filtro $L(z)$ calculado como em \eqref{eq:dbcd_vrft_filter_l_vrft}, o valor assintótico de $\theta$ pode ser
escrito como:
...
de dados coletados com a planta em laço fechado, podem ser encontrados em \cite{lecchini}.
...
\begin{itemize}
	...
	\item {\it{Identificação da planta:}} Identifica-se a planta $G_0(z)$ com uma família de modelos como $G(z,\theta_G)$ a
...
Os parâmetros $\theta_0$ deste sistema é definido por $[ 0.4 \; -0.68 \; 0.288 ]$.
...
exemplo onde a classe de modelos do controlador escolhida não consegue representar o controlador ideal $C_d(z)$.
