...
\begin{itemize}
  \item O sistema não é afetado por ruído, ou seja, $\sigma^2=0$.
...
\begin{equation}
J_{MR}(\theta) = \left \| \left ( \frac{G_0(z)C(z,\theta)}{1+G_0(z)C(z,\theta)} -T_d(z) \right )W(z)
\right \|_2^2 
\label{eq:vrft_method_cost_func}
\end{equation} 

Sendo $T_d(z)$ o modelo de referência a ser atingido em malha fechada quando o controlador
obtido é aplicado sobre a planta e $W(z)$ é uma matriz para ponderação escolhida pelo usuário.

Métodos diretos são conceitualmente mais naturais que os indiretos, mas mesmo com o apelo que possuem,
...
propicia que estas duas equações tenham mínimos muito próximos.
...
Utilizando as definições de $u_L(t)$, $\varepsilon_L(t)$ e dada a definição de $C_0(z)$ juntamente com o
...
Obtém-se com a utilização do filtro, uma estimativa onde o erro de polarização é minimizado.
%===============================================================================
\subsection{Dados corrompidos por ruído}
\label{sec:dbcd_vrft_framework_noise}
%===============================================================================

Todos os equacionamentos até aqui apresentados consideraram que o sistema não fosse afetado por
ruído. Nesta seção será apresentado brevemente o comportamento do método quando o sinal $y(t)$ é
...
\hline
        ...
        $J_{MR}(\theta)$ & Variância estimativas $\theta$   \\
\hline
...
$J_{VR}^N(\theta) = 2.5008\times10^{-5}$ e $J_{MR}(\theta) = 1.7746\times10^{-5}$ além de uma variância para as
...
$C_0(z) \in \mathcal{C(z, \theta)}$. Nesta seção será apresentado um exemplo onde o modelo do controlador escolhido
