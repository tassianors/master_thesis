...
\begin{itemize}
  \item O sistema não é afetado por ruído, ou seja, $\sigma^2(t)=0$.
...
com a planta trabalhando em laço aberto. \cite{campi_leccini_savaresi2002}. Para a ideia estendida
...
ser compreendido analisando a expressão do critério $J_{VR}(\theta)$ quando utiliza-se o sinal
$\tilde{y}(t)$:


\begin{equation}
J_{VR}(\theta)= \frac{1}{2\pi} \int_{-\pi}^{\pi} \left [ \left | G \right |^2 \left | C(\theta)-C_d
\right |^2 \left | 1-M \right |^2 \frac{\left | L \right |^2}{\left | M \right |^2} \Phi_u +\frac{\left | 
C(\theta) \right |^2}{\left | G \right |^2 \left | C_d \right |^2}\left | L \right |^2\Phi _d
\right ] \; d\omega
...
Os parâmetros do controlador $\hat{\theta}^{IV}_N $ podem então ser calculados como em
...
assintoticamente que $\hat{\theta}^{IV}= \hat{\theta}$, entretanto um experimento adicional é
...
\begin{itemize}
	...
	sinal simulado $\hat{y}=\hat{G}(z)u(t)$, obtendo a variável instrumental como:
...
Utilizando o método do VRFT para identificação do controlador quando este está submetido a um ruído $\sigma
_\upsilon ^2=0.005$ obtém-se a estimativas dos parâmetro s $\theta_1$ e $\theta_2$ apresentados na Figura
...
Na figura (\ref{fig:vrft_bj_M10_var02}) quadruplicou-se a variância do ruído inserido no sistema ($\sigma
_\upsilon ^2=0.02$). Observa-se então que o erro de polarização existe na estimativa. Como descrito em
\cite{campi_leccini_savaresi2002} quando o método do VRFT é utilizado com ruído nas amostras, a estimativa é
inevitavelmente polarizada. Neste mesmo trabalho é sugerido a utilização de {\it{variáveis instrumentais}}
...
Utilizando o procedimento descrito na seção (\ref{sec:dbcd_vrft_framework_noise}) para dados corrompidos por ruído
o resultado obtido, para a mesma variância de $\sigma_\upsilon ^2=0.02$ do ruído, é apresentado na Figura
...
Tabelas (\ref{table:vrft_method_bj}) e (\ref{table:vrft_method_bj_iv}) onde o custo $J_{y}$ (equação
\eqref{eq:vrft_method_cost_func}) e o custo $J_{VR}^N$ (equação \eqref{eq:vrft_method_filter_criter_assim})
são apresentados para diferentes valores de variância do ruído para o mesmo sistema BJ.

\begin{table*}[htbp]
\begin{center}
\caption{Valor dos custos $J_{VR}^N$ e $J_{y}$ além da  variância das
estimativas para diferentes valores de $\sigma _\upsilon ^2$ quando o método
VRFT não utiliza variáveis instrumentais para a estimativa dos parâmetros
$\theta$}
...
\caption{Valor dos custos $J_{VR}^N$ e $J_{y}$ além da  variância das
estimativas para diferentes valores de $\sigma _\upsilon ^2$ quando o método
VRFT utiliza variáveis instrumentais para a estimativa dos parâmetros $\theta$}
...
quando comparado com o método onde não são utilizadas variáveis instrumentais. Demonstrando assim que o
...
controlador quando não são utilizados variáveis instrumentais. Obteve-se desta forma um custo
$J_{VR}^N(\theta) = 2.5008\times10^{-5}$ e $J_{y}(\theta) = 1.7746\times10^{-5}$ além de uma variância para as
...
\begin{figure}[htbp] 
	...
	apresentado em \eqref{eq:vrft_methos_ex_arx_c} com variância do ruído $\sigma_\upsilon ^2=0.005$}
...
nas estimativas. Desta forma as informações apresentadas a seguir serão feitas utilizando variáveis instrumentais.
Na figura (\ref{fig:vrft_arx_M10_var05_iv}) é apresentado a estimativa dos parâmetros do controlador para um
ruído com variância $\sigma_\upsilon ^2=0.05$. Observa-se que não há erro de polarização nas estimativas. 
...
\begin{figure}[htbp] 
	...
	\caption{100 estimativas Monte Carlo dos parâmetros $\theta_1$, $\theta_2$ e $\theta_3$ para o controlador
	apresentado em \eqref{eq:vrft_methos_ex_arx_c} com variância do ruído $\sigma_\upsilon ^2=0.05$ utilizando
...
o comportamento desejado $T_d(z)$ faz parte da classe escolhida para a identificação. Em outras palavras quando
$C_d(z) \in \mathcal{C(z, \theta)}$. Nesta seção será apresentado um exemplo onde o modelo do controlador escolhido
para o sistema não consegue levar este para $T_d(z)$, ou seja, não consegue representar a totalidade das dinâmicas de
$C_d(z)$.
...
\begin{figure}[htbp] 
	...
	ruído de variância $\sigma_\upsilon ^2=0.1$}
