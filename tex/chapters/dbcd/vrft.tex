...
global do critério escolhido, sob condições ideais. Ele irá buscar o mínimo da função custo dentro das limitações de
excitabilidade do sinal utilizado. Se o sinal utilizado para exitar o sistema for suficientemente informativo, então o
minimo encontrado pelo método será um minimo global.

Assume-se que a planta do sistema é {\it{linear}} SISO ({\it{single input single output}}) de tempo discreto,
descrito pela função de transferência racional $G_0(z)$. Tal que esta função de transferência é desconhecida e
...
transferência é dada por $M(z)$. Desta forma, se $M(z)$ for excitado com qualquer sinal $r(t)$,
...
\begin{itemize}
	...
	adquirindo-se a saída $\tilde{y}(t)'$. Como o ruido entre um experimento e outro é independente,
...
_\upsilon ^2=0.005$ obtém-se a estimativas dos parâmetro s $\rho_1$ e $\rho_2$ apresentados na Figura
...
\begin{figure}[htbp]
	...
	\caption{Resultado das 100 estimativas de Monte Carlo dos parâmetros $\rho_1$ e $\rho_2$ para o controlador
...
\begin{figure}[htbp] 
	...
	\caption{100 estimativas Monte Carlo dos parâmetros $\rho_1$ e $\rho_2$ para o controlador apresentado em
	\eqref{eq:vrft_methos_ex_bj_c} com variância do ruido de 0.02}
	\label{fig:vrft_bj_M10_var02}
\end{figure}

Utilizando o procedimento descrito na seção (\ref{sec:dbcd_vrft_framework_noise}) para dados corrompidos por ruido
...
\begin{figure}[htbp]
	...
	\caption{Resultado das 100 estimativas de Monte Carlo dos parâmetros $\rho_1$ e $\rho_2$ para o
	controlador apresentado em \eqref{eq:vrft_methos_ex_bj_c} com variância do
	ruido de 0.02. Utilizando variáveis instrumentais para estimar os parâmetros.}
...
= 5.1242$ e a variância dos parâmetros estimados foi de $0.5064\times10^{-6}$ para $\rho_1$ e de
...
são apresentados para diferentes valores de variância do ruido para o mesmo sistema BJ.
...
\caption{Valor dos custos $J_{VR}^N$ e $J_{MR}$ além da  variância das
estimativas para diferentes valores de $\sigma _\upsilon ^2$ quando o método
VRFT utiliza variáveis instrumentais para a estimativa dos parâmetros $\rho$}
...
\begin{figure}[htbp] 
	...
	\caption{100 estimativas Monte Carlo dos parâmetros $\rho_1$, $\rho_2$ e $\rho_3$ para o controlador
	apresentado em \eqref{eq:vrft_methos_ex_arx_c} com variância do ruido $\sigma_\upsilon ^2=0.005$}
...
\begin{figure}[htbp] 
	...
	\caption{100 estimativas Monte Carlo dos parâmetros $\rho_1$, $\rho_2$ e $\rho_3$ para o controlador
	apresentado em \eqref{eq:vrft_methos_ex_arx_c} com variância do ruido $\sigma_\upsilon ^2=0.05$ utilizando
...
\caption{Valor dos custos $J_{VR}^N$ e $J_{MR}$ além da  variância das
estimativas para diferentes valores de $\sigma _\upsilon ^2$ quando o método
VRFT utiliza variáveis instrumentais para a estimativa dos parâmetros $\rho$ do controlador
...
para o sistema não consegue levar este para $M(z)$, ou seja, não consegue representar a totalidade das dinâmicas de
$C_0(z)$.
...
comportamento desejado para o sistema em malha fechada ($M(z)$) é possível fazer um comparativo da resposta
...
primeiro muito mais próximo da função $M(z)$ desejada.
