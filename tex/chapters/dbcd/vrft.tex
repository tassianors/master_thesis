...
estes dois métodos pertencendo a uma mesma classe, algumas diferenças significativas existem e podem ser ressaltadas:
\cite{campi_savaresi2000}
...
representar exatamente o controlador ótimo ($C_d(z)$), ou seja:
...
\mathcal{C}$ as duas funções custo são iguais, possuindo também o mesmo mínimo. Quando $C_d(z) \notin \mathcal{C}$
...
Entretanto esta não é a única escolha para a aproximação de $S(z,\theta)$; o método VRFT usa invariavelmente o seguinte:
...
$J_{VR}(\theta)$ se aproxima do valor de mínimo de $J_y(\theta)$ com a utilização do filtro $L(z)$. Também observa-se que a curva de
...
modelo. O uso do filtro $L(z)$ reduz o erro de polarização.}
...
\nonumber
\end{equation}
e que este termo não decai a zero mesmo que $C_d(z) \in \mathcal{C}$. Desta forma é esperado que haja erro de
...
\begin{itemize}
	...
	Devido às incertezas na estimativa de $G(z,\theta_G)$, este segundo método não garante que a
...
função $T_d(z)$ desejada. Os custos destes dois sistemas são apresentados na Tabela \ref{table:vrft_method_notinclass}.
...
\begin{figure}[htbp] 
	...
	sem o filtro $L(z)$ e variáveis instrumentais.}
