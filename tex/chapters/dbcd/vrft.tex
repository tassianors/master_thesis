...
estes dois métodos pertencendo uma a mesma classe, algumas diferenças são ressaltadas: \cite{campi_savaresi2000}
...
\begin{itemize}
	...
	a planta, podendo inclusive utilizar os dados do funcionamento normal desta.
\end{itemize}
...
exata resposta desejada pela escolha de $T_d(z)$.
...
C(z,\theta)$ as duas funções custo são iguais, possuindo também o mesmo mínimo. Quando$C_d(z) \notin C(z,\theta)$ nenhum
dos critérios desaparece e o ponto de mínimo depende de vários fatores dentro da integral. Se os fatores dentro dos dois
...
assintoticamente que $\theta_0^{IV}= \theta_0$, entretanto um experimento adicional é
requisitado. O segundo não garante, rigorosamente, $\theta_0^{IV}= \theta_0$ mas o erro
...
Nas identificações apresentadas a seguir será sempre utilizado um sina PRBS de ordem 7.
...
ruído é quadruplicado ($\sigma_e^2=0.02$). Observa-se então que o erro de polarização existe na estimativa. Como
...
\begin{figure}[htbp] 
	...
	\eqref{eq:vrft_methos_ex_bj_c} com variância do ruído de 0.02}
...
\begin{tabular}{cccc}
\hline
        Variância $\sigma_e^2$ & $J_{VR}^N(\theta)$ & $J_y(\theta)$ & Variância $\theta$   \\
\hline
	    0.06    & 1.7893$\times10^{-2}$ & 8.2367$\times10^{-3}$ & $1.0\times10^{-5}\;\left[ 0.4754\;\;0.4434\right ]$ \\
	    0.05    & 1.2515$\times10^{-2}$ & 5.5366$\times10^{-3}$ & $1.0\times10^{-5}\;\left[ 0.2671\;\;0.3244\right ]$ \\
        0.04    & 8.1897$\times10^{-3}$ & 3.6071$\times10^{-3}$ & $1.0\times10^{-5}\;\left[ 0.1534\;\;0.1583\right ]$ \\
        0.01    & 4.9665$\times10^{-4}$ & 2.4402$\times10^{-4}$ & $1.0\times10^{-6}\;\left[ 0.0963\;\;0.1035\right ]$ \\
        0.005   & 1.2515$\times10^{-4}$ & 5.6013$\times10^{-5}$ & $1.0\times10^{-7}\;\left[ 0.2999\;\;0.3114\right ]$ \\
        0.001   & 5.0036$\times10^{-6}$ & 3.5734$\times10^{-6}$ & $1.0\times10^{-8}\;\left[ 0.1301\;\;0.1223\right ]$ \\
\hline
\end{tabular}
\end{center}
\end{table*} 
...
\begin{tabular}{cccc}
\hline
        Variância $\sigma_e^2$ & $J_{VR}^N(\theta)$ & $J_{y}(\theta)$ & Variância $\theta$   \\
\hline
	    0.06    & 45.1719  &  $9.7345\times10^{-5}$ & $1.0\times10^{-5}\;\left[ 0.5161 \;\; 0.5332\right ]$ \\
	    0.05    & 33.2600  &  $2.0457\times10^{-5}$ & $1.0\times10^{-5}\;\left[ 0.2481 \;\; 0.2652\right ]$ \\
        0.04    & 21.2652  &  $1.1665\times10^{-4}$ & $1.0\times10^{-5}\;\left[ 0.2040 \;\; 0.2084\right ]$ \\
        0.01    & 1.2956   &  $8.9695\times10^{-6}$ & $1.0\times10^{-6}\;\left[ 0.1246 \;\; 0.1138\right ]$ \\
        0.005   & 0.3264   &  $7.4764\times10^{-6}$ & $1.0\times10^{-7}\;\left[ 0.3063 \;\; 0.2917\right ]$ \\
        0.001   & 0.0126   &  $5.2443\times10^{-7}$ & $1.0\times10^{-8}\;\left[ 0.1059 \;\; 0.1017\right ]$ \\
\hline
\end{tabular}
\end{center}
\end{table*}
...
quando comparado com o método onde não são utilizadas variáveis instrumentais, cooroborando com o que foi apresentado na
