...
critério de desempenho \eqref{eq:dbcd_def_track_error}, replicado abaixo:
...
Feedback Tuning}}). Mesmo estes dois métodos pertencendo a uma mesma classe, algumas diferenças significativas existem e
...
\begin{itemize}
	...
	desempenho \eqref{eq:dbcd_def_track_error}. Este método não requer experimentos específicos sobre
...
Com o intuito de exemplificar, na Figura \ref{fig:vrft_cost_functions} é apresentada uma curva da função custo
...
Considerando que a função custo $J_{y}(\theta)$ apresentada em \eqref{eq:dbcd_def_track_error} e o
critério do método de referência virtual $J_{VR}(\theta)$ apresentado em \eqref{eq:vrft_method_algorithm_criter} são
diferentes fora de situações ideais, a escolha correta do filtro $L(q)$ propicia que estas duas equações tenham
mínimos próximos \cite{campi_leccini_savaresi2002}.
...
\mathcal{C}$ as duas funções custo possuem o mesmo mínimo. Quando $C_d(q) \notin \mathcal{C}$
...
na curva encontrado pelo algoritmo seja menos preciso, aumentando a variância das estimativas, enquanto reduz o erro
de polarização.
...
Para que haja rejeição a este ruído no método, em \cite{campi_leccini_savaresi2002} foi sugerido a
...
Utilizando o método do VRFT para identificação do controlador quando este está submetido a um ruído
com variância $\sigma_e^2=0.005$ obtém-se as estimativas dos parâmetros $\theta_1$ e $\theta_2$ apresentados na Figura
\ref{fig:vrft_bj_M10_var005} com um resultado de 100 simulações com $N=2540$ cada.
...
estimativas. O custo para estas e outras, estimativas são apresentados na Tabela \ref{table:vrft_method_arx_iv}.
...
%===============================================================================
\subsubsection{Controlador não pertence à classe}
