...
Onde o ruido é modelado por um polinômio que pode ou não ser linear. A consideração básica 
...
Claramente a convergência do algoritmo depende da convergência da estimativa da sequencia do ruído.
\cite{billings_zhu91} 
...
Os parametros da equação \eqref{eq:nl_alg_buck_rational} são os valores de refencia para a utilização do
...
o processo real e pela escolha do modelo de rúido que para este caso foi apenas utilizado o erro da
...
\begin{equation}
\text{variância }\;\theta =\begin{bmatrix}
...
\begin{equation}
\text{Covariância de }\;\theta = 1.0\times 10^{-3}\begin{bmatrix}
...
Para o sistema real utilizou-se os valores de referência: $\theta= $
...
Utilizando par a simulação um sinal PRBS de 635 pontos e um ruído de variancia $\sigma^2=0.005$ os resultados
...
\begin{equation}
\text{variância }\;\theta =1.0\times 10^{-4} \begin{bmatrix}
...
estimativas dos parametros $\theta_1$ e $\theta_2$ e a elipse de confiança para $\chi^2=95\%$.
