...
Em muitas aplicações praticas de controle, uma descrição nmatematica da planta não é disponível e o
controlador deve ser projetado baseado nas medidas obtidas deste sistema. Este assunto tem atraido a
atenção de diversos engenheiros de controle desde 1940 com o pioneiro trabalho de Ziegler e Nichols
(1942) em ajuste de controladores PID industriais. Depois deste trabalho de Ziegler e Nichols
divesos trabalhos surgiram, muitos em formas de aperfeiçoamento e extensões das tecnicas ja
apresentadas, e algumas com desenvolvimento em novas direções (\cite{mcmillan1983tuning},
\cite{Haalman1965}). A caracteristica principal destes metodos é que eles podem
ser facilmente implementados: Simples experimentos sobre a planta são executados e simples regras 
são aplicadas sobre os dados obtidos. O metodo VRFT que será abordado aqui tem algumas destas 
caracteristicas, pois só requer que um experimento sobre a planta seja executado. 
...
filtro $L_u$. Com $T_0(\rho)$ e $S_0(\rho)$ sendo representados como sendo a função de transferência
em malha fechada e a função sensibilidade, respectivamente.
...
Onde o primeiro termo é o erro de seguimento de referência, o segundo é a contribuição do disturbio
e o terceiro é o esforço de controle.

Para minimizar \eqref{eq:vrft_db_ift_j_cost_parts}, busca-se encontrar um ponto
estacionário da equação, para isso calcula-se o gradiente desta euqação e aplica-se o
algoritmo para a convergência \eqref{eq:vrft_db_ift_algoritm}. \cite{hajalmson_ift_non_linear}
...
$R_i$ é uma matriz positiva definida, tipicamente uma estimativa da Hessiana de $J$, como a
aproximação de Gauss-Newton.

Para cada iteração $i$ em \eqref{eq:vrft_db_ift_algoritm} IFT usa dois experimentos, cada um de
tamanho $N$. 

%===============================================================================
\subsection{CbT - Correlation Based Tuning}
\label{sec:vrft_control_cbt}
%===============================================================================
