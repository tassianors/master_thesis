...
global do critério escolhido. Isso só não é  verdade sempre pois esta afirmativa depende das limitações de
qualidade e excitabilidade do sinal utilizado para se adquirir os dados utilizados no processo. Dentro destas
limitações é possível afirmar que o método VRFT irá procurar pelo menor mínimo possível. Se o sinal utilizado
...
diretamente para identificar o controlador e não há necessidade de identificação da planta para
...
Utilizando as definições de $u_L(t)$, $e_L(t)$ e dada a definição de $C_0(z)$ juntamente com o
...
Todos os equacionamentos até aqui apresentadas consideraram que o sistema não seja afetado por
...
\begin{itemize}
	...
	sinal simulado $\hat{y}=\hat{G}u(t)$, obtendo a variável instrumental como:
...
\begin{figure}[htbp]
	...
	\caption{100 estimativas Monte Carlo dos parâmetros $\rho_1$ e $\rho_2$ para o
...
= 5.1242$ e a variância dos parâmetros estimados foi de $0.5064e-06$ para $\rho_1$ e de $0.5495e-06$ para
$\rho_2$.
...
Como já foi observado a utilização de variáveis instrumentais melhora significativamente o erro existente nas
estimativas. Desta forma as informações apresentadas a seguir serão feitas utilizando variáveis instrumentais.
...
comportamento desejado para o sistema em malha fechada ($M(z)$) é possível fazer um comparativo da resposta de
