...
Todos os equacionamentos até aqui apresentadso, consideraram que o sistema não era afetado por
...
Assume-se que o sinal $u(t)$ e $\xi(t)$ são descorrelacionados e também que os dados são coletados
...
ser compreendido analizando a expressão do critério $J_{VR}(\theta)$ quando se utiliza o sina
$\tilde{y}(t)$:


\begin{equation}
J_{VR}(\theta)= \frac{1}{2\pi} \int_{-pi}^{\pi} \left [ \left | G \right |^2 \left | C(\theta)-C_0 \right
|^2 \left | 1-M \right |^2 \frac{\left | L \right |^2}{\left | M \right |^2} \Phi_u +\frac{\left | 
...
Os parametros do controlador $\hat{\theta}^{IV}_N $ pode então ser calculado como em
...
que se espera é pequeno, além disso um segundo experimento não é necessário.
\cite{campi_leccini_savaresi2002}
