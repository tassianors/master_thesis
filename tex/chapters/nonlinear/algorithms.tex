...
&&  +c(e(t-1), ... , e(t-n_e)) +e(t)
\end{eqnarray}
onde  $a(\cdot)$ e $b(\cdot)$ são polinômios e é conveniente definir o numerador e denominador de
como em:
...
o instante $t-1$. Desta forma o vetor de parâmetros a ser estimados possui tamanho $N_n + N_d$.

A equação \eqref{eq:nl_alg_rational} possui não linearidade nos parâmetros, tornando a
identificação mais complexa por não ser possível a utilização do método dos mínimos quadrados para a
estimativa dos parâmetros. Para isso será apresentado a seguir um algoritmo iterativo que fará a identificação dos
parâmetros do sistema.

Uma das maiores desvantagens do modelo racional quando comparado com o modelo polinomial é que o modelo polinomial é
linear nos parâmetros. Muitos resultados para identificação de sistemas lineares podem ser estendidos para modelos
polinomiais não lineares e várias rotinas de determinação de estrutura já foram desenvolvidas para isso
\cite{chen_billings1989}.


%===============================================================================
\subsection{Algoritmo para identificação de modelos racionais}
\label{sec:nl_si_algorithms_rationals}
%===============================================================================

Esta seção descreve um algoritmo para determinar os parâmetros de modelos racionais \eqref{eq:nl_alg_rational}. Este
algoritmo foi proposto por \cite{correa} e é uma modificação do algoritmo originalmente proposto por 
\cite{billings_zhu}. 
...
%==========================================================================
% Step 1
\item Faça $i=0$. Monte a matriz $\Psi$ de regressão e estime os parâmetros usando
...
A convergência do algoritmo depende da convergência da estimativa da sequência do ruído \cite{billings_zhu91}.
...
Utilizando para simulação como entrada $u(t)$ um sinal PRBS de 635 pontos e adicionando-se um ruído $e(t)$ de variância
$\sigma_e^2=0.005$ os resultados da média de 100 estimativas de Monte Carlo foram:
...
Devido à presença de ruído e ao erro de polarização apresentado ser menor que a variância do ruído adicionado, sabe-se
...
O sistema real $\mathcal{S}$ não consegue ser representado pela classe de modelos escolhida:
...
Utilizando para simulação um sinal PRBS de 635 pontos e um ruído de variância $\sigma_e^2=0.005$ os resultados
...
A fim de comparar a qualidade dos resultados obtidos, na Figura \ref{fig:nl_rat_example3} são apresentadas as respostas
...
Como não há um valor de $\theta_0$ para comparação, visto que o sistema real não consegue ser representado pelo modelo,
...
\end{equation}
onde $\alpha = 0.8872$, $\beta = 1.2$ e $R=33$ são constantes que dependem apenas dos componentes do
...
modelo com estrutura {\it{ad hock}}, que aproxima relativamente bem a característica estática do
...
\ref{table:nl_alg_buck} são apresentados os valores estimados utilizando o algoritmo de identificação de sistemas
racionais NARMAX para o sistema apresentado em \eqref{eq:nl_alg_buck_circ}. Ao utilizar-se 200 pontos e um valor inicial de $31.5$
obteve-se o seguinte conjunto de parâmetros $\theta$:
...
\begin{figure}[htbp]
	...
	quando os parâmetros estimados para um valor inicial de 31.5, como apresentado em
	\eqref{eq:nl_rat_buck_theta_init31.5}.}
...
\begin{table*}[htbp]
\begin{center}
\caption{Tabela com os valores das estimativas do conversor de corrente contínua para diferentes valores de condições
...
É interessante observar que os parâmetros estimados $\theta$ variam consideravelmente com as alterações dos valores
...
inferiores a $23.25$ o custo da estimativa diverge, fazendo com que os parâmetros estimados não mais consigam
