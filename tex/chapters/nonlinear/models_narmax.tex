...
de $u(t)$ e $e(t)$. $\theta$ é o vetor de parametros a ser identificado.

As classes de modelos NARMAX podem então ser compreendidas como um conjunto ou somatório de funções não lineares, que
podem ser parametrizados linearmente. Vale aqui esclarescer que a forma apresentada em 
\eqref{eq:nlin_models_narmax_generic} não é uma regressão linear, onde tem-se:
...
partir dos dados coletados do sistema.

Observa-se desta forma que as funções radiais de base apresentadas na seção (\ref{sec:nl_models_radiais}) por meio da
equação \eqref{eq:nl_models_rbf} são um caso particular de sistemas NARX.
