...
Famílias ou conjuntos de modelos para sistemas nãolineares podem ser divididos em dois grupos principais, baseados na
...
afinidade em representar certos tipos de não linearidades. Enende-se por afinidade a caracteristica de conseguir
representar o sistema com um número de parêmetros menor que outro modelo com outra base e que use mais parametros para
conseguir representar este sistema.
...
\ref{sec:si_exper_overfit} foram apresentadas algumas idéias relacionadas a este assunto.

Juntamente com a escolha do modelo para representar o sistema não linear, saber se este é identificável é de grande
importância. Em \cite{glad_ljung} é apresentado um procedimento para determinar a identificabilidade de sismtena não
lineares.
...
Na Figura (\ref{fig:nl_models_hammerstein_wiener}) observa-se o diagrama de blocos para os modelos
...
sistema e isso inevitavelmente significa que que um conjunto tipicamente elevado de parâmetros é necessário para
...
\end{equation}
onde $b$ é conhecido como {\it{bias}} e $\omega_j$ são constantes e $\emph{f}(\cdot)$ é chamada função de ativação. A
...
\caption{Representação de redes neurais recorrentes. Onde $z^{-1}$ representa um atraso de tempo na realimentação do
sistema.}
