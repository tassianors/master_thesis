...
Geralmente, na modelagem de sistemas algumas considerações são feitas sobre o
este:
...
superposição}}.  A consideração da
...
Uma das desvantagens deste modelo é a falta de liberdade para descrever as propriedades
...
As equações que descrevem a saída do sistema em função da entrada $u(t)$ e o ruido $e(t)$ como apresentado em \eqref{eq:si_modeling_lti}
e que podem ser caracterizados pela utilização de vários polinômios, nada mais são do que famílias de modelos $\mathcal{M}$.
Existem para cada família de modelo uma infinidade de possíveis saídas para uma mesma entrada, bastando para isso que os
parâmetros dos coeficientes dos polinômios sejam escolhidos apropriadamente. O objetivo na identificação de sistemas é encontrar um conjunto de
coeficientes que consiga melhor descrever os dados observados na saída, para uma determinada entrada.

Da mesma forma que com um modelo pode-se obter diversas saídas diferentes, vários modelos diferentes podem chegar a uma mesma 
saída. Este é de certa forma um problema para a escolha do modelo que será utilizado, pois se ambos chegam teoricamente a mesma
resposta, qual dos modelos é o melhor? Este é um dos motivos pelos quais a correta escolha de um modelo, ou família de modelos, é
importante.

Escolher um modelo que não consegue representar o sistema físico propicia erros na estimativa dos parâmetros. (Mais informações sobre
estes erros serão abordados na seção (\ref{sec:si_par_estim_uncertanties})). Por outro lado a super estimativa da ordem do modelo
pode adicionar complexidade em desnecessária além de comportamentos transientes no modelo que não existem na planta real.
