...
Considere o sistema apresentado em (\ref{eq:si_modeling_lti}). Assume-se que para um preditor de um passo a
frente os sinais $y(p)$ e $u(p)$ são conhecidos para $p \le t-1$. A partir de
...
tem-se que até $\upsilon (p)$ é definido para $p \le t-1$. Como $e(t)$ pode ser estimado por

\begin{equation}
e(t)=\widetilde { H } (q)\upsilon (t)
\label{eq:si_par_estim_e}
\end{equation}

O objetivo então é prever $y(t)$ como em:
...
Fazendo-se as substituições necessárias chega-se ao estimador 

\begin{equation}
\hat { y } (t|t-1)={ H }^{ -1 }(q)G(q)u(t)+\left[ 1-{ H }^{ -1 }(q) \right] y(t)
\label{eq:si_par_estim_predict}
\end{equation}

Enfatiza-se a dependência com o parâmetro $\theta$ \cite{ljung}:
...
Este preditor também é conhecido como preditor ótimo, sendo utilizado em diversos métodos de identificação de
sistemas.

O erro de predição é a diferença entre os valores atingido pelo preditor e os valores 
reais coletados do sistema como apresentado em:
...
A equação do erro (\eqref{eq:si_par_estim_err_predic} pode ser reescrita como:
...
Desta forma introduz-se: .\cite{ljung}
...
Chega-se então à seguinte equação para o erro de predição:

\begin{equation}
\varepsilon (t,\theta)=y(t)-\theta^T\varphi (t,\theta)
\label{eq:si_estim_predict_erro_epsilon}
\end{equation}
...
O modelo apresentado em (\ref{eq:si_obj_single_var}) é facilmente estendido para o modelo
...
A partir de (\ref{eq:si_par_estim_err_predic}) e (\ref{eq:si_obj_single_var}) temos 
...
que minimiza a função custo (\ref{eq:si_obj_etim_lsm_v}). Este é o critério de minimização do 
erro de predição para o método dos mínimos quadrados.

O valor de $\hat{\theta}$ que minimiza (\ref{eq:si_obj_single_var}) é então dado
por:\cite{system_identification}
...
erros de polarização na estimativa, a variável de regressão $\phi(t)$ não pode estar correlacionada com o
