...
Diversos métodos de projetos de controladores baseados em dados que otimizam algum critério de performance apareceram na
literaatura, com diferentes focos para diferentes critérios. Estes critérios expressam um ou uma combinação de objetivos
...
(\cite{campi_leccini_savaresi2002, campi_savaresi2006}) é baseado na manipulação de variáveis que transforma um critério
$H_2$ de performance em um quadrático nos parametros. A função custo quadrática resultante pode ser minimizado
diretamente, ou seja, nenhuma iteração é necessária. Entretanto apenas o objetivo de seguimento de referência é tratato
(a não ser que um controlador com dois níveis de liberdade seja usado \cite{lecchini_campi_savaresi_2dof}) e o minimo
...
um método iterativo que usa a ideia de variáveis instrumentais para remover o indesejado efeito do ruído enquanto busca
seu objetivo de seguimento de referência. \cite{Bazanella_h2criteria2008} 

Otimização baseada em dados para um critério de performance genérico $H_2$ aparece em \cite{Hjalmarsson1994}, onde um
método para obter estimativas não polarizadas diretamente do gradiente da função custo apartir de dados do sistema em
malha fechada é proposto. Este método é chamado de {\it{Iterative Feedback Tuning}} (IFT). IFT é discutido a fundo em
...
destacados os blocos do controlador, da planta e da funçaõ que modifica o ruído branch $e(t)$.
