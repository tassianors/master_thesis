...
aproximar esta função por outro polinômio de ordem 4, definido por $\vartheta (t)$:
...
%===============================================================================
Nesta seção serão apresentados alguns exemplos do algoritmo apresentado na Seção
\ref{sec:nl_si_algorithms_rationals} em conjunto com o principio básico do método VRFT para a obtenção dos
sinais $r(t)$ e $e(t)$, além do sinal escolhido $u(t)$ utilizado para excitar a planta e o próprio sinal de
saída da planta $y(t)$.
...
Escolhendo um conjunto de modelos que consegue representar o controlador ótimo:
...
Utilizando um sinal PRBS de tamanho 254 e o método do VRFT para gerar os sinais $r(t)$ e $e(t)$. 
Adicionando-se um ruido de variância $\sigma^2 = 0.005$, a média da estimativa para 100 experimentos de
Monte Carlo foi de:
...
\begin{equation}
\text{Covariância }\theta=1.0\times 10^{-6}\begin{bmatrix}
...
é apresentado na figura \ref{fig:vrft_nl_dynamic_step_erro}.
...
E a fim de ilustrar as estimativas obtidas pelo método, na Figura \ref{fig:vrft_nl_dynamic_t1_t2} são
...
O custo $J_{VRFT}=2.7291\times10{-8}$ foi obtido utilizando-se os sinais de saída do controlador obtido e
esperado. Já o custo entre os sinais de saída do sistema em malha fechada estimado e desejado foi de
$J_{MR}=0.0033$.

Aumentando a variância para $\sigma^2 = 0.01$ os resultados obtidos foram:
...
\begin{equation}
\text{Desvio padrão de }\theta=\begin{bmatrix}
...
\begin{equation}
\text{Covariância }\theta=1.0\times 10^{-4}\begin{bmatrix}
...
Com os custos de $J_{VRFT}=1.9177\times10{-6}$ e $J_{MR}=0.0148$, corroborando para o fato de que os custos não foram
significativamente afetados pelo aumento do ruído. Fazendo assim com que a estimativa ainda seja confiável.
...
posso usar este. esta tudo pronto. só escrever.
% vrft13_nl_rat_SisnotinM_2.m
