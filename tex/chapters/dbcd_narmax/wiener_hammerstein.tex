...
ou Hammerstein, descritos na Seção \ref{sec:nl_models_wiener_hammerstein}. Será utilizado o conceito de referência virtual
...
Analogamente $G'(q)$ e $C'(q)$ são os blocos lineares da planta e do controlador. Como a não linearidade do controlador está na entrada,
este poderia ser caracterizado como uma classe de Hammerstein, mas será identificado por uma classe de modelos
polinomial, não deixando entretanto de ainda ser representável por um modelo de Hammerstein.
...
\usefont{T1}{ppl}{m}{n}
\rput(4.5507812,-0.59){$\mathcal{C}$}
\usefont{T1}{ppl}{m}{n}
\rput(9.670782,-0.59){$\mathcal{S}_{nl}$}
\usefont{T1}{ppl}{m}{n}
\rput(3.3407812,0.71){$\gamma(\cdot)$}
\usefont{T1}{ppl}{m}{n}
\rput(10.820781,0.71){$\zeta(\cdot)$}
\usefont{T1}{ppl}{m}{n}
\rput(5.610781,0.71){$C'(q)$}
\usefont{T1}{ppl}{m}{n}
\rput(8.620781,0.71){$G'(q)$}
\end{pspicture} 
}
...
Espera-se que o sistema em malha fechada se comporte linearmente como $T_d(q)$, desta forma o controlador ótimo
...
\begin{equation}
\gamma (r(t) - \zeta(\omega(t))) = r(t) - \omega(t) 
...
\begin{equation}
\gamma (\epsilon(t)) = \zeta(\omega(t))^{-1}
\nonumber 
\end{equation}

$\gamma (\cdot)$ como descrito em \eqref{eq:dbnarmac_ex_wiener_gamma} pode não ser parametrizável por um modelo
polinomial. Aproxima-se $\gamma (\cdot)$ então por uma classe de modelos NARMAX a fim de tornar a identificação do controlador
