...
{\cal D}\rightarrow \cal D$, com $\cal D\subset \Re$, é analitico. A Figura \ref{fig:Wiener} representa este sistema.
...
constantes, a não linearidade inversa $\gamma(\cdot)$ pode não ser autonoma, violando outra pressuposição da formulação
...
Fica claro que qunado não existe um sinal de referência, $r(t) \equiv 0$, a relação entre os polinomios $\gamma(\cdot)$ e
...
Para aproveitar esta virtude computacional é necessario ter um controlador parametrizavel linearmente. Desde
que a inversa é assumidamente analitica, ela pode ser espandida em serie de Taylor e então truncada:
...
\end{eqnarray}
com $D(q)$ sendo um polinomio mônico conhecido, $N(q)\bydef [1 \quad q^{-1} q^{-2} \ldots
...
\eqref{eq:track_error_controller_v} com $C(\theta, q)$ como um controlador do tió proporcional integral (PI).

\begin{equation}
C(\theta, q)=\theta_1\frac{1}{q-1}+\theta_2\frac{q}{q-1}
\label{eq:dbnarmax_ex_wiener_controller_class_pi}
\end{equation}  
e a função não linear sendo representada por um polinomio de quarta ordem como:
...
representado por $C(\theta, q)$, ou seja, $C_d(q) \notin C(\theta, q)$. Se pro ventura o desempenho do controlador
...
média da diferênça entre o sinal de saída obtido e o desejado. Para as estimativas foram utilizados sinais de entrada do
