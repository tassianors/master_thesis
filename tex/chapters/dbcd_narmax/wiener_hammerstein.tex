...
\label{fig:dbnarmax_controller_hammerstein}
\end{figure}

Realmente, se $\gamma(\cdot ) = \zeta_R^{-1}(\cdot )$ e 
\begin{equation}
C_d' (q)= \frac{T_d(q)}{G_0(q)(1-T_d(q))}
\label{eq:dbcdnl_wiener_lin_cd}
\end{equation}
...
\eqref{eq:dbcdnl_wiener_lin_cd}, resultando em 
...
\begin{figure}[htbp] 
	\center 
	\includegraphics[width=0.95\columnwidth]{figures/wiener_step.eps}
...
não linearidades $\gamma(\epsilon(t))$ e $\zeta(z(t))$ quando o sistema é alimentado por uma referência unitária.

\begin{figure}[htbp] 
	\center 
	\includegraphics[width=0.95\columnwidth]{figures/wiener_v_and_z_signals.eps}
...
\begin{figure}[htbp] 
	\center 
	\includegraphics[width=0.95\columnwidth]{figures/wiener_vxz.eps}
