...
sendo $\zeta(t)$ e $\iota(t)$ os blocos não lineares da planta e do controlador respectivamente. Análogamente
...
Obtem-se então o relacionamento entre os sinais $\iota (t) $ e $\zeta(t)$:

\begin{equation}
( r(t) - \omega(t) \zeta(t)) \iota (t) = r(t) - \omega(t) 
\nonumber
\end{equation}
...
\begin{equation}
\iota (t) = \frac{r(t)-\omega(t)}{r(t) - \omega(t)\zeta(t)} 
\end{equation}

Pecebe-se claramente que quando não há sinal de referência $r(t)$, o relacionamento entre os polinomios $\iota (t) $
e $\zeta(t)$ fica simplificado a:

\begin{equation}
\iota (t) = \frac{1}{\zeta(t)}
\nonumber 
\end{equation}

Define-se então $\iota (t)$ como sendo:

\begin{equation}
\iota (\epsilon(t))= v(t) = a_1\epsilon (t) + a_2 \epsilon ^2(t) +a_3 \epsilon ^3(t) +a_4 \epsilon ^4(t)
...
Como a estimativa de $\iota (\epsilon(t))$ é apenas uma aproximação do que espera-se ser a inversa de
...
não linearidades $\iota(\epsilon(t))$ e $\zeta(\omega(t))$ respectivamente.
...
Observa-se que a relação destes sinais é praticamente linear, garantido que a aproximação de $\iota(\epsilon(t))$
