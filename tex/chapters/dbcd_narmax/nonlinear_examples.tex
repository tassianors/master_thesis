...
Pecebe-se claramente que quando não há sinal de referência $r(t)$, ou seja

\begin{equation}
r(t) \equiv 0
\nonumber 
\end{equation}
o relacionamento entre os polinomios $\gamma(t) $ e $\zeta(t)$ fica simplificado a:
...
Determinar análiticamente a equação de $\gamma (t)$ como descrito em \eqref{eq:dbnarmac_ex_wiener_gamma} é uma tarefa um
...
Onserva-se que $u(t)$ em \eqref{eq:dbnarmax_ex_wiener_controller} é um modelo NARMAX polinomial. Foram realizados 100
...
\begin{figure}[htbp] 
	...
	controlador não linear representado por um modelo NARMAX polinômial é inserido na planta.}
...
\eqref{eq:dbnarmax_ex_wiener_gamma_choosen} foi suficietne para representar com acuracidade o que era esperado para
...
Inicialmente um exemplo onde o controlador que leva a planta $G_0(z)$ a se comportar em malha fechada como em $T_d(z)$,
...
$\epsilon(t)$. Adicionando-se um ruído de variância $\sigma_e^2 = 0.005$ a saída do sistema, a média das estimativas
...
Para as simulações foi adicionado um ruído com $\sigma_e^2 = 0.05$. Foram realizados 100 experimentos de Monte Carlo
