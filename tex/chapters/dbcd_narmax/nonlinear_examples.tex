...
O comportamento do sistema em malha fechada esperado é definido por $T_d(z)$:
...
Espera-se então que por $T_d(z)$ ser linear, que o controlador tenha um bloco que cancele o efeito não linear de
$\zeta(t)$.
...
levaria a planta em malha fechada a ter o comportamento descrito por $T_d(z)$ é:
...
O custo entre os sinais de saída do sistema obtido e o sistema esperado $T_d(z)$ foi de  $J_{MR}(\theta)=
...
$T_d(z)$ inicialmente escolhida. Para as estimativas foram utilizados sinais de entrada do tipo PRBS com 127 pontos para a
excitação da planta.
