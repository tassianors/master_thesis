...
Nesta seção serão apresentados os resultados obtidos para a identificação de sistemas que podem ser descritos por
modelos NARMAX racionais ou polinomiais. Os sinais de entrada do algoritmo de identificação serão obtidos por meio de
referência virtual.

Os exemplos se dividem de acordo com o tipo de não linearidade. Na Seção \ref{sec:dbnarmax_nl_wiener} é apresentado um
exemplo onde a não linearidade está acoplada estáticamente na saída da planta. Na Seção \ref{sec:dbnarmax_nl_dinamic}
são apresentados exemplos onde a não linearidade está na dinâmica do processo. Os exemplos nesta seção estão divididos
entre controladores ideias que conseguem ser representados pela classe de modelos escolhida ($C_d(z) \in C(z,\theta)$) e
quando a classe de modelos não consegue representar a totalidade das dinâmicas presentes ($C_d(z) \notin C(z,\theta)$).
...
de modelos NARMAX racional, não deixando entretanto de ainda ser representável por um modelo de Hammerstein.
...
Identificação de controladores que possuem não linearidades na dinâmica do seu modelo serão apresentados nesta seção.
Busca-se com isso apresentar a proposta de identificação de controladores não lineares que possam ser descritos por
classes de modelos NARMAX.

Serão apresentados exemplos onde o controlador ideal está na classe de modelos escolhida e também exemplo quando as
dinâmicas do controlador ideal não conseguem ser representadas totalmente pelo modelo escolhido.
...
%===============================================================================
\subsubsection{Controlador ideal não sendo representado pela classe de modelos: $C_d \notin C(z,\theta)$}
