...
o sismtema real que se quer identificar é o controlador ideal definido por $C_d(z)$. Define-se desta forma a classe de
...
\label{sec:dbnarmax_controller}
%===============================================================================

Para a metodologia proposta neste trabalho, tem-se que a planta $G_0(z)$ é não linear e que o comportamento do sistema
em malha fechada $T_d(z)$ escolhido para o sistema é linear. Tem-se desta forma que o controlador ideal $C_d(z)$ será
obrigatóriamente não linear para fazer com que as não linearidades da planta não sejam observadas na saída do processo
depois que este for posto para operar em malha fechada.

Desta forma, para que a classe de controladores consiga representar a totalidade das dinâmicas de $C_d(z)$ é necessário
que exista um conjunto de parametros $\theta_0$ tal que:

\begin{equation}
C(z,\theta_0)=C_d(z) 
\nonumber
\end{equation}
se isso não for observado então a classe escolhida não é capaz de representar a totalidade das dinâmicas do controlador
ideal e a sua estimativa será polarizada, assim como no caso de $\mathcal{S} \notin \mathcal{M}$ apresentado no
Capítulo \ref{chapter:system_identification}.
