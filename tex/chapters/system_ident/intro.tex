...
$e(t)$.  Sendo $u(t)$ o sinal de entrada aplicado sobre a planta e $y(t)$ o sinal
...
O objetivo da identificação de sistemas é encontrar um valor para o vetor $\theta$ que faça com que $G(q,
\theta)$ seja o mais próximo possível de $G_0(q)$. O vetor $\theta$ que faz com que estas duas funções de
transferências sejam iguais é denotado por $\theta^*$. Segue então que:
...
A identificação de sistemas é dependente de três componentes básicas: Primeiro delas é o sistema real observado,
definido pela simbolo $\mathcal{S}$ e que pode ser definido a partir de \eqref{eq:si_intro_system} como abaixo:
...
Segunda componente é a classe de modelos escolhida, denotada por $\mathcal{M}$:
...
Usualmente o critério mais utilizado é o de erro de predição. 

Preditores são equações que predizem qual será o próximo valor de saída do sistema
baseado no conjunto do modelo e nos valores de dados coletados até aquele momento.
Os sinais $y(t)$ e $u(t)$ são os sinais da saída e entrada medidas do sistema real enquanto que o sinal
$\hat{y}(t, \theta)$ é o sinal de saída do preditor. A diferênça entre o valor do preditor e o valor do sistema real,
...
predição. Existe um processo o qual se deseja estimar, para isso escolhe-se uma classe de modelos onde os
parametros a serem ajustados são representados por $\theta$. O preditor é então ajustado, baseado nas diferentes
possíbilidades de escolha de $\theta$. Este ajuste é feito a partir do erro entre o sistema real $y(t)$ e a saída do
...
Assumindo que $G(0, \theta)=0$, $H(0,\theta)=I$ e que $H^{-1}(q, \theta)$ e $H^{-1}(q,\theta)
G(q,\theta)$ são assisntóticamente estáveis, se $u(t)$ e $e(p)$ para $p<t$ não forem correlacionados,
...
\begin{equation}
V(\theta)=\frac{1}{2}\sum_{t=1}^{N}\varepsilon ^2(t, \theta)=\frac{1}{2}\varepsilon^T\varepsilon=\frac{1}{2}\left \|
\varepsilon \right \|
...
quadratico entre os dados obtidos do sistema real $\mathcal{S}$ e os dados obtidos por meio do modelo estimado.
