...
Considerando o sistema linear apresentado em \eqref{eq:si_intro_model} e assumindo que $G(0, \theta)=0$, $H(0,\theta)=1$
e que $H^{-1}(z, \theta)$ e $H^{-1}(z,\theta) G(z,\theta)$ são assintoticamente estáveis, se $u(t)$ e $e(p)$ para $p<t$
não forem correlacionados, então o preditor ótimo pode ser apresentado como: \cite{system_identification}
