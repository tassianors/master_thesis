...
O erro de polarização só é nulo se $\nu(t)$ for ruido branco ou se $E(\varphi(t)) = 0$. Esta desvantagem do
método dos mínimos quadrados pode ser visto como uma oportunidade para a introdução do método de variáveis instrumentais.
...
A opção de modelagem do ruido pode ser utilizado para a obtenção de estimativas não polarizadas, mesmo quando
utiliza-se o método dos mínimos quadrados e o ruído sobre o sistema não é branco.

A classe de modelos definida na equação \eqref{eq:si_intro_model} apresenta além da classe de modelos para a planta
$G(q, \theta)$ o modelos do sistema que atua sobre o ruído $H(q, \theta)$. Desta forma considera-se que o sistema possui
um ruído aditivo que não é branco, também chamado de ruído colorido.

É simples observar que um modelo que leve em consideração não apenas o modelo da planta, mas também a modelagem do ruído
atuante sobre este será mais completo.   
