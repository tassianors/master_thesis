...
A identificação de sistemas é formada por três premissas básicas: o sistema real $\mathcal{S}$, a classe de modelos
...
custo, conhecida também como critério. Na seção \ref{sec:sys_ident_intro} foi apresentado o critério de minimização mais
...
ao ano de 1809 utilizado por Gauss para determinação da órbita dos planetas \cite{system_identification}. Método este
chamado de mínimos quadrados (MMQ).
...
\eqref{eq:si_method_criteria}. O valor de $\hat{\theta}$ que minimiza esta função custo é dada por:
...
\begin{itemize}
  ...
  muito alto, com relação a ordem do sistema. O que implica que $A(z)$ e $B(z)$ possuem fatores comuns. Podendo
  ocasionar dinâmicas no modelo que não existem no sistema real.
...
chamado de {\it{bias}} ou erro de polarização da estimativa. Para contornar este erro existem duas opções que serão
abordadas em seguida: utilização de variáveis instrumentais e inclusão da modelagem do ruído à classe de modelos do
sistema.
...
a necessidade de um conjunto maior de dados para que a estimativa convirja para o valor de $\theta^*$.
\end{enumerate}

Ao escolher as variáveis instrumentais é importante notar que a escolha não deve ser apenas para evitar 
a correlação entre o vetor de erro e os instrumentos. A razão para isso é que os intrumentos 
...
para a realização da estimativa, o que só é possível quando o denominador de $G(z,\theta)$ e $H(z,\theta)$ são iguais.
...
\nonumber
\end{equation}
o sistema não é canonicamente parametrizável quando todos os polinômios são diferentes do caso unitário ($A(z),\ldots,
...
ponto de mínimo da função custo seja proporcional a complexidade da curvatura da função.
