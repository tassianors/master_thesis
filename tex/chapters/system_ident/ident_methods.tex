...
Nesta seção serão apresentados métodos que minimizam a função custo \eqref{eq:si_method_criteria}. 
...
Um preditor pode serdefinido como:
...
Onde $\hat{y}(t)$ é a predição, $\varphi (t)$ é comumente chamado de {\it{variável de regressão}} e $\theta$ é o vetor de
...
Quando o número de amostras tende ao infinito e a estimativa tende para um valor diferente de $\theta_0s$, este erro é
chamado de {\it{bias}} ou erro de polarização da estimativa.

Para contornar este erro de {\it{bias}} existem duas opções que serão abordadas em seguida: Utilização de variáveis
...
variáveis instrumentais trará uma estimativa com uma variância maior que o método dos minimos quadrados, pois a
...
O método dos mínimos quadrados possui erro de polarização quando existe correção entre o regressor $\varphi(t)$ e
$\nu(t)$, apresentou-se em seguida uma alternativa ao método dos minimos quadrados que resolve este inconveniente ao
custo da escolha de uma variável instrumental que não seja correlacionada com o ruído.

Existe entretanto uma segunda forma para lidar com este erro de polarização: modelagem do ruído. Em algumas aplicações,
a modelagem do sistema é o objetivo principal da identificação, para tanto leva-se em conta apenas os parametros de
$G(q, \theta)$. Desta forma é possível que haja polarização dos parâmetros, pois o sistema real $\mathcal{S}$ não
consegue ser descrito pelo modelo apenas com $G(q, \theta)$. Como apresentado na equação \eqref{eq:si_intro_model} e
aqui replicado:
...
O modelo so sistema deve conter o modelo do ruído para que o sistema possa ser completamente representado, evitando
assim erro de polarização.

Ao integrar ao modelo do sistema, os parametros de $H(q, \theta)$, surge o inconvenientede que se o denominador de
$G(q, \theta)$ for diferente do dominador de $H(q, \theta)$ então o sistema não é canonicamente parametrizavel e com
isso a função custo não é convexa, adicionando um grau considerável aos algoritmos para encontrar o mínimo global da
equação. \cite{system_identification}
