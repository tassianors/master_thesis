...
a média móvel $C(q,\theta)$ do ruído $e(t)$. Na Figura \ref{fig:si_models_diagram_narmax} é apresentado o diagrama de
...
\usefont{T1}{ppl}{m}{n}
\rput(4.3745313,-1.89){$C(q)$}
\usefont{T1}{ppl}{m}{n}
\rput(2.7645311,1.21){$\frac{B(q)}{A(q)}$}
\usefont{T1}{ppl}{m}{n}
\rput(5.574531,3.11){$\frac{C(q)}{A(q)}$}
...
Quando apenas um conjunto dos polinômios de $A(q),\ldots, F(q)$ é utilizado, obtém-se diferentes tipos de estruturas
