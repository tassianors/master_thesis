...
único vetor $\theta$ que consiga representar melhor o sistema real $\mathcal{S}$ de acordo como critério de performance
escolhido.
...
espectro do sinal de entrada, $\Phi _u(\omega)$, for diferente de zero em pelo menos $n$ pontos no intervalo $-\pi<
...
necessária é $n_b+n_f$.

Para um experimento em malha aberta, considerando um sistema ARMAX ({\it{Autoregressive, autoregressive model with
exogenous input}}) e {\it{Box-Jenkins}} o experimento é suficientemente informativo se a ordem de excitação for 
$n_b+n_a$. Em \cite{gevers_bazanella2009} foram apresentadas as condições necessárias e suficientes para informatividade
em estruturas de modelos arbitrárias.
