...
SISO descrito por (\ref{eq:si_intro_model}) tendo a função de transferência $G(z,\theta)$ a
função racional apresentada em (\ref{eq:si_data_g_rational}):

\begin{equation}
G(z,\theta)=\frac{B(z,\theta)}{F(z,\theta)}=\frac{z^{n_k}(b_1+b_2z^{-1}+...+b_{nb}z^{-nb+1})}{1+f_1z^{-1}+...+f_{nf}z^{-nf}}
...
Um aspecto comumente não desejado para aplicações de engenharia é quando a solução do problema não possui apenas uma
...
simples e consistem em algumas perguntas que o projetista do experimento de identificação deve fazer:
