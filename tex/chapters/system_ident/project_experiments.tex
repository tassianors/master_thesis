...
A forma mais simples para a gerar este sinal é, baseado em um ruido Gausiano branco
...
semelhantes a de ruido branco. Este sinal é gerado pela equação \eqref{eq:si_project_signal_prbs}.
...
As vantagens e desvantagens do sinal PRBS comparado com um ruido randômico binário pode ser
listado: \cite{ljung}
...
\item A variância assintótica depende tipicamente dos parâmetros do sistema $\theta^*$, $P=P(\theta^*)$ que são
desconhecidos.
\end{enumerate}
...
\begin{equation}
P^{-1}(\theta^*)=\frac{1}{2 \pi \lambda_0}\int_{-\pi}^{\pi}\mathcal{F}(e^{j\omega}, \theta^*)\begin{bmatrix}
\Phi_u(\omega) & \Phi_{ue}(\omega)\\ 
\Phi_{ue}^{*}(\omega) & \lambda_0 
\end{bmatrix}\mathcal{F}(e^{j\omega}, \theta^*)\;d\omega
