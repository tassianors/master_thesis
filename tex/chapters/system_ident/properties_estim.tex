...
Nesta seção tem-se o objetivo de descrever algumas das propriedades estatísticas relevantes para o identificação de
sistemas. Tem-se o intuito de formalizar quais são os erros que podem estar contidos nas estimativas obtidas e quais são
as condições para evita-las, ou reduzi-las.
...
quão confiável esta informação é, não é suficiente para a maioria das aplicações. 
...
certo montante de dados serão levados junto para quando $N \to \infty$, é natural que as condições sobre os dados, sejam
...
que exita um atraso de tempo tanto em $G_0(z)$ e $G(z,\theta)$. Então:
...
Se em adição, a classe de modelos é globalmente identificável em $\theta_0 \in D_{\mathcal{M}}$, então:
...
introduzir o conceito de {\it{erro de variância}} e {\it{erro de polarização}}.
...
\begin{equation}
\hat{Q}_N(z)=\begin{bmatrix}
G(z, \hat{\theta}_N) & H(z, \hat{\theta}_N)
\end{bmatrix}^T
\nonumber
\end{equation}

A qualidade da estimativa pode ser calculada em termos da diferença entre o processo real $Q_0(z)$ e o 
melhor sistema que o modelo pode atingir (quando a quantidade de dados é ilimitada ($N\rightarrow \infty$)) tem-se
então a estimativa do modelo $Q_N(z)$.\cite{campestrini, solari}

\begin{equation}
\Delta Q_N(z) \equiv \hat{Q}_N(z)-Q(z)
...
\begin{equation}
Q^*(z) = \left [ G(z, \theta^*) \; H(z, \theta^*) \right ]^T
\label{eq:si_par_estim_under_q*}
\end{equation}

Entende-se por $Q^*(z)$ como a melhor aproximação que o método pode proporcionar quando a quantidade de dados coletados
$N \rightarrow \infty$.

Adicionando e subtraindo a equação \eqref{eq:si_par_estim_under_q*} na equação \eqref{eq:si_par_estim_under_diff}
chega-se à definição dos erros de polarização e de variância:

\begin{equation}
\Delta Q_N(z) \equiv \underset{\text{Erro de variância}}{\underbrace{\hat{Q}_N(z)-Q^*(z)}}+  \underset{\text{Erro de
polarização}}{\underbrace{Q^*(z)-Q(z)}}
...
do ruído $H(\cdot)$ \cite{ljung}. O conjunto de modelos: 

\begin{equation}
\mathcal{G}=\left \{ G(e^{j\omega},\theta) \;|\; \theta \in D_{\mathcal{M}} \right \}
\nonumber
\end{equation}
é abrangente o suficiente pra que 

\begin{equation}
G_0(z) \in \mathcal{G}
\end{equation}
mas a função de transferência do filtro real do ruído $H_0(z)$ não consegue ser totalmente descrito pela classe de
...
e se $G(z, \theta)$ e $H(z, \theta)$ são parametrizados de forma independente:
...
\nonumber
\end{equation}
onde $H_*(z)$ é um filtro do ruído fixo, onde $\mathcal{S} \notin \mathcal{M}$.

Este modelo também cobre o caso da estrutura de modelos Box-Jenkins (Tabela \ref{table:si_modeling_models}).
Consequentemente esta estrutura de modelos tem a vantagem de que a função de transferência $G(z, \theta)$ pode ser
...
Uma vez tendo entendido as propriedades de convergência de $\hat{\theta}_N$, a pergunta que surge é o quão rápido esta
...
distribuição probabilística. $\hat{\theta}_N -\theta^*$ tipicamente decai a $1/\sqrt{N}$. Então a distribuição da
variável randômica:
...
\nonumber
\end{equation}
irá convergir para uma distribuição bem definida Gausiana sobre suposições fracas. \cite{ljung}
...
Para o caso onde $\mathcal{S} \in \mathcal{M}$ e assumindo ainda que $\varepsilon(t,\theta_0)=e_0(t)$ é uma sequencia
...
\end{equation}
onde $\mathcal{F}(z, \theta_0)=\left [ \mathcal{F}_u(z, \theta_0) \;\;\;\mathcal{F}_e(z, \theta_0) \right ]$ e:
...
\end{equation}
é a região de confiança, onde assintoticamente inclui $\theta_0$ com probabilidade $\alpha$. Desta forma tem-se que as
estimativas estarão centradas em $\hat{\theta}_N$ e com probabilidade $\alpha$ estarão contidas em um elipsoide definido
por $P_N$ e $\chi_{\alpha}^2(N)$ \cite{jansson}.
...
\begin{figure}[htbp]
	...
	\caption{Estimativas de um sistema e a elipse reprsentando a região de confiança para um $\chi$ de 99\%. No centro da
	elipse destaca-se a médida de todas as estimativas.}
