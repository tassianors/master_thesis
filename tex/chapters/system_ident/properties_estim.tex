...
Nesta seção tem-se o objetivo de descrever algumas das propriedades estatisticas relevantes para o identificação de
sistemas. Tem-se o intuíto de formalizar quais são os erros que podem estar contidos nas estimativas obtidas e quais são
as condições para evita-las, ou reduzi-las.

Conhecer a qualidade das estimativas que são obtidas de um processo de identificação e suas propriedades estatisticas
...
\end{equation}
é o ponto de partida básico para a identificação de sistemas. As propriedades calculadas de $\hat{\theta}_N$ para um
certo montante de dados serão levados para quando $N \to \infty$, é natural que as condições sobre os dados, sejam
relativos a um conjunto infinido $Z^{\infty}$. \cite{ljung}
...
que exita um atrazo de tempo tanto em $G_0(z)$ e $G(z,\theta)$. Então:
...
do ruído $H(\cdot)$. O conjunto de modelos: \cite{ljung}
...
G_0(z) \in \mathcal{G}
\end{equation}
mas a função de transferência do filtro real do ruído $H_0(z)$ não consegue ser totalmete descrito pelo conjunto de
modelos. Desta forma $\mathcal{S} \notin \mathcal{M}$ e supondo que o conjunto de dados $Z^{\infty}$ esteja sugeito ao
que foi descrito na seção (\ref{sec:si_par_estim_data_z}) e que este seja suficientemente informativo.
Se o experiemnto é realizado em malha aberta, ou seja, quando $u(t)$ e $e(t)$ são descorrelacionados
e se $G(z, \theta)$ e $H(z, \theta)$ são parametrizados de forma indenpente:
...
O caso da parametrização independente em \eqref{eq:si_par_estim_independent_param} cogre o caso do {\it{Output Error}}
...
\nonumber
\end{equation}
onde $H_*(z)$ é um filtro do ruído que faz com que  $\mathcal{S} \notin \mathcal{M}$.
...
%===============================================================================
\subsection{Distribuição assintótica da estimativas dos parâmetros}
...
que é uma variável randomica, e seu ``tamanho'' pode ser caracterizado pela sua matriz de covariancia, ou pela sua
distribuição probabilistica. $\hat{\theta}_N -\theta^*$ típicamente decai a $1/\sqrt{N}$. então a distribuição da
variável randomica:
...
randômica independênte de média zero e variância $\lambda_0$. Desta forma tem-se
...
Asssim quando o sistema real pertence à classe  de modelos escolhida a estimatica converge para $\theta_0$ e a covariância da
...
sistema real. Outro ponto a observar é que o espectro cruzado $\Phi_{ue}$ é zero quando o sistema esta operando em laço
aberto. \cite{jansson}
