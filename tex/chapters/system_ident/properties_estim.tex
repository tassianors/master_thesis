...
que é uma variável randomica, e seu ``tamanho'' pode ser caracterizado pela sua matriz de covariancia, ou mais
completamente, pela sua distribuição probabilistica. $\hat{\theta}_N -\theta^*$ típicamente decai a $1/\sqrt{N}$. então
a distribuição da variável randica 
...
Assume-se que dentro do conjunto $D_{\mathcal{M}}$ exista apenas um $\theta^*$. Em \eqref{eq:si_par_estim_under_theta}
foi apresentado que $\hat{\theta}_N \to \theta^*$ quando $N \to \infty$ e considerando que :
...
onde $P(\theta)$ é dado por:

\begin{equation}
P(\theta_0)\overset{\underset{\mathrm{\Delta}}{\,}}{=}\lim_{N\to \infty}N\;E(\hat{\theta}_N-\theta^*)(\hat{\theta}_N-\theta^*)^T
\label{eq:si_par_estim_P_theta}
\end{equation}

Assume-se ainda que $\varepsilon(t,\theta_0)=e_0(t)$ é uma sequencia randômica independênte de média zero e variância
$\lambda_0$

%preciso colocar a quação inicial do 9.3 a 9.6 no inicio deste capituklo para dai conseguir escrever a 9.17 que vailogo
% em seguida.
...
Neste caso, $\mathcal{N}(0, P_\theta)$ denota uma distribuição normal com média 0 e matriz de covariância $P_\theta$.
